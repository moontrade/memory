...
func (an *artNode) setPrefix(key []byte, prefixLen uint32) *artNode {
	node := an.node()
	node.prefixLen = prefixLen
	for i := uint32(0); i < min(prefixLen, MaxPrefixLen); i++ {
		node.prefix[i] = key[i] // key.Byte(int(i))
	}
...
func (l *leaf) match(key Key) bool {
	if key.Pointer == 0 || l.key.Len() != key.Len() {
		return false
	}
