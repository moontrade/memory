...
func TestTreeLongestCommonPrefix(t *testing.T) {
	...
	l1 := factory.newLeaf(keyOf("abcdefg12345678"), "abcdefg12345678").leaf()
	l2 := factory.newLeaf(keyOf("abcdefg!@#$%^&*"), "abcdefg!@#$%^&*").leaf()
	assert.Equal(t, uint32(7), tree.longestCommonPrefix(l1, l2, 0))
	assert.Equal(t, uint32(3), tree.longestCommonPrefix(l1, l2, 4))

	l1 = factory.newLeaf(keyOf("abcdefg12345678"), "abcdefg12345678").leaf()
	l2 = factory.newLeaf(keyOf("defg!@#$%^&*"), "defg!@#$%^&*").leaf()
...
func TestTreeInsertSimilarPrefix(t *testing.T) {
	...
	tree.Insert(keyOfBytes([]byte{1}), 1)
	tree.Insert(keyOfBytes([]byte{1, 1}), 11)

	v, found := tree.Search(keyOfBytes([]byte{1, 1}))
	assert.True(t, found)
	assert.Equal(t, 11, v)
}
...
func TestTreeInsert3AndSearchWords(t *testing.T) {
	...
	for _, term := range searchTerms {
		v, found := tree.Search(keyOf(term))
		assert.True(t, found)
...
func TestTreeInsertAndGrowToBiggerNode(t *testing.T) {
	...
	for _, data := range testData {
		...
		for i := byte(0); i < data.totalNodes; i++ {
			tree.Insert(keyOfBytes([]byte{i}), i)
		}
...
func TestTreeInsertWordsAndMinMax(t *testing.T) {
	...
	tree := newTree()
	for _, w := range words {
		tree.Insert(keyOfBytes(w), w)
	}

	for _, w := range words {
		v, found := tree.Search(keyOfBytes(w))
...
func (ds *testDataset) processDatasetAsBytes(t *testing.T, tree *tree, bts []byte) {
	for _, term := range bts {
		...
		val, deleted := tree.Delete(keyOfBytes([]byte{term}))
		...
		_, found := tree.Search(keyOfBytes([]byte{term}))
...
func (ds *testDataset) processDatasetAsStrings(t *testing.T, tree *tree, strs []string) {
	for _, term := range strs {
		val, deleted := tree.Delete(keyOf(term))
		assert.Equal(t, ds.deleteStatus, deleted, ds.message)
		...
		_, found := tree.Search(keyOf(term))
		assert.False(t, found, ds.message)
	}
}
	...
	var data = []*testDataset{
			...
			true},
		{
			...
			testDatasetBuilder(func(data *testDataset, tree *tree) {
				for i := byte(0); i < 49; i++ {
					tree.Insert(keyOfBytes([]byte{i}), keyOfBytes([]byte{i}))
				}
			}),
			[]byte{byte(123)},
			49,
			...
			true},
		{
			...
			testDatasetBuilder(func(data *testDataset, tree *tree) {
				for i := byte(0); i < 49; i++ {
					term := []byte{i}
					val, deleted := tree.Delete(keyOfBytes(term))
			...
			true},
		{
			...
			testDatasetBuilder(func(data *testDataset, tree *tree) {
				for i := 0; i < 256; i++ {
					...
					tree.Insert(keyOfBytes(term.Bytes()), keyOfBytes(term.Bytes()))
				}
			}),
			[]byte{2},
			255,
...
func TestDeleteOneWithSimilarButShorterPrefix(t *testing.T) {
	tree := newTree()
	tree.Insert(keyOf("keyb::"), "0")
	tree.Insert(keyOf("keyb::1"), "1")
	tree.Insert(keyOf("keyb::2"), "2")

	v, deleted := tree.Delete(keyOf("keyb:"))
	assert.Nil(t, v)
...
func TestInsertAndDeleteOne(t *testing.T) {
	tree := newTree()
	tree.Insert(keyOf("test"), "data")
	v, deleted := tree.Delete(keyOf("test"))
	...
	tree = newTree()
	tree.Insert(keyOf("test"), "data")
	v, deleted = tree.Delete(keyOf("wrong-key"))
	assert.Nil(t, v)
	assert.False(t, deleted)
}
...
func TestInsertTwoAndDeleteOne(t *testing.T) {
	tree := newTree()
	tree.Insert(keyOf("2"), 2)
	tree.Insert(keyOf("1"), 1)

	_, found := tree.Search(keyOf("2"))
	assert.True(t, found)
	_, found = tree.Search(keyOf("1"))
	assert.True(t, found)

	v, deleted := tree.Delete(keyOf("2"))
	assert.True(t, deleted)
	if deleted {
		assert.Equal(t, 2, v)
	}
...
func TestTreeTraversalPreordered(t *testing.T) {
	...
	assert.Equal(t, traversal[1].Key(), keyOf("1"))
	assert.Equal(t, traversal[1].Value(), 1)
	assert.Equal(t, Leaf, traversal[1].Kind())

	assert.Equal(t, traversal[2].Key(), keyOf("2"))
...
func TestTreeTraversalNode48(t *testing.T) {
	...
	for i := 48; i > 0; i-- {
		tree.Insert(keyOfBytes([]byte{byte(i)}), i)
	}
	...
	for i := 1; i < 48; i++ {
		assert.Equal(t, traversal[i].Key(), []byte{byte(i)})
...
func TestTreeTraversalPrefix(t *testing.T) {
	...
	for _, d := range dataSet {
		...
		for _, k := range d.keys {
			tree.Insert(keyOf(k), string(k))
		}

		actual := []string{}
		tree.ForEachPrefix(keyOf(d.keyPrefix), func(node Node) bool {
			if node.Kind() != Leaf {
				return true
			}
			k := node.Key()
...
func TestTreeTraversalForEachPrefixWithSimilarkeyOf(t *testing.T) {
	tree := newTree()
	tree.Insert(keyOf("abc0"), "0")
	tree.Insert(keyOf("abc1"), "1")
	tree.Insert(keyOf("abc2"), "2")

	totalKeys := 0
	tree.ForEachPrefix(keyOf("abc"), func(node Node) bool {
		if node.Kind() == Leaf {
			totalKeys++
		}
		return true
...
func TestTreeTraversalForEachPrefixConditionalCallback(t *testing.T) {
	tree := newTree()
	tree.Insert(keyOf("America#California#Irvine"), 1)
	tree.Insert(keyOf("America#California#Sanfrancisco"), 2)
	tree.Insert(keyOf("America#California#LosAngeles"), 3)

	totalCalls := 0
	tree.ForEachPrefix(keyOf("Amer"), func(node Node) (cont bool) {
		if node.Kind() == Leaf {
			totalCalls++
		}
		return true
...
func TestTreeTraversalForEachCallbackStop(t *testing.T) {
	tree := New()
	tree.Insert(keyOf("0"), "0")
	tree.Insert(keyOf("1"), "1")
	tree.Insert(keyOf("11"), "11")
	tree.Insert(keyOf("111"), "111")
	tree.Insert(keyOf("1111"), "1111")
	tree.Insert(keyOf("11111"), "11111")
	...
	tree = New()
	for _, w := range words {
		tree.Insert(keyOfBytes(w), string(w))
	}
	...
	tree.ForEach(func(node Node) (cont bool) {
		totalCalls++
		k := node.Key()
		if k.String() == "A" { // node48 with maxChildren?
			return false
		}
		return true
	})
	...
	tree.ForEach(func(node Node) (cont bool) {
		totalCalls++
		k := node.Key()
		if k.String() == "Aani" { // node48 'a' children?
			return false
		}
		return true
	})
...
func TestTreeTraversalForEachPrefixCallbackStop(t *testing.T) {
	...
	totalCalls := 0
	tree.ForEachPrefix(keyOf("0"), func(node Node) (cont bool) {
		totalCalls++
		return false
	})
	assert.Equal(t, 1, totalCalls)
	...
	totalCalls = 0
	tree.ForEachPrefix(keyOf("11"), func(node Node) (cont bool) {
		totalCalls++
		return false
	})
	assert.Equal(t, 1, totalCalls)
	...
	totalCalls = 0
	tree.ForEachPrefix(keyOf("nokey"), func(node Node) (cont bool) {
...
func TestTreeTraversalPrefixWords(t *testing.T) {
	...
	foundWords := []string{}
	tree.ForEachPrefix(keyOf("antisa"), func(node Node) bool {
		if node.Kind() != Leaf {
			return true
		}
...
func TestTreeIterator(t *testing.T) {
	tree := newTree()
	tree.Insert(keyOf("2"), []byte{2})
	tree.Insert(keyOf("1"), []byte{1})
...
func TestTreeInsertAndDeleteAllWords(t *testing.T) {
	...
	for _, w := range words {
		v, deleted := tree.Delete(keyOfBytes(w))
		assert.True(t, deleted)
		assert.Equal(t, w, v)
	}
...
func TestTreeAPI(t *testing.T) {
	...
	oldValue, deleted := tree.Delete(keyOf("non existent key"))
	...
	value, found := tree.Search(keyOf("non existent key"))
	assert.Nil(t, value)
	assert.False(t, found)
	...
	// test tree with data
	tree = New()
	oldValue, updated := tree.Insert(keyOf("Hi, I'm Key"), "Nice to meet you, I'm Value")
	assert.Nil(t, oldValue)
	assert.False(t, updated)
	assert.Equal(t, 1, tree.Size())
	...
	oldValue, updated = tree.Insert(keyOf("Hi, I'm Key"), "Ha-ha, Again? Go away!")
	...
	tree.ForEach(func(node Node) bool {
		assert.Equal(t, keyOf("Hi, I'm Key"), node.Key())
	...
	oldValue, updated = tree.Insert(keyOf("Hi, I'm Value"), "Surprise!")
	assert.Nil(t, oldValue)
	assert.False(t, updated)

	tree.Insert(keyOf("Now I know..."), "What?")
	tree.Insert(keyOf("ABC"), "ABC")
	tree.Insert(keyOf("DEF"), "DEF")
	tree.Insert(keyOf("XYZ"), "XYZ")
...
func TestTreeInsertAndSearchKeyWithNull(t *testing.T) {
	...
	tree.ForEach(func(node Node) bool {
		k := node.Key()
		traversal = append(traversal, string(k.String()))
		return true
	}, TraverseLeaf)
...
func TestNodesWithNullKeys48(t *testing.T) {
	...
	// delete all terms
	for _, term := range terms {
		v, deleted := tree.Delete(keyOf(term))
		assert.True(t, deleted)
		assert.Equal(t, term, v)
	}
...
func TestNodesWithNullKeys256(t *testing.T) {
	...
	for _, term := range terms {
		_, updated := tree.Insert(keyOf(term), term)
		assert.False(t, updated)
	}
...
func TestTreeInsertAndSearchKeyWithUnicodeAccentChar(t *testing.T) {
	...
	tree.Insert(keyOfBytes([]byte(a)), a)
	tree.Insert(keyOfBytes(accent), string(accent))

	v, found := tree.Search(keyOfBytes([]byte("a")))
	assert.True(t, found)
	assert.Equal(t, a, v)

	v, found = tree.Search(keyOfBytes(accent))
	assert.True(t, found)
